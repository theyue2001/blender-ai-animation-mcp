...
TILT=math.radians(45.0)     # dome axis tipped toward camera, out of the screen plane
C0=Vector((0.0,493.0,0.0)); R=8.5
TOCAM=Vector((0,1,0))       # camera sits at +Y
rep={"tilt_deg":round(math.degrees(TILT),1)}
DIRS={"A":Vector((0,0,1)),
...
for k in "ABC":
    ...
    d=(DIRS[k].normalized()*math.cos(TILT)+TOCAM*math.sin(TILT)).normalized()
    DP[k]=d
    ...
    rep.setdefault("dome_dir",{})[k]=[round(v,3) for v in (Ral.to_3x3()@u)]
    rep.setdefault("want",{})[k]=[round(v,3) for v in d]
...
cam.data.lens=LENS; cam.data.dof.focus_distance=D
rep["camera"]={"D":round(D,2),"loc":[round(camX,3),round(C0.y+D,2),round(camZ,3)],
               "sx":[round(min(xs),2),round(max(xs),2)],"sz":[round(min(zs),2),round(max(zs),2)],
               "depth":[round(min(q.y for q in corners),2),round(max(q.y for q in corners),2)]}
